...
optimize.features<-function(set,ranking,num=200,step=45,replication=5,list=FALSE,parallel=TRUE,cores=8){
  l1<-length(set[[2]])
  l2<-length(ranking)
  if(l1!=l2)
  ...
  if(parallel==TRUE){
    ...
    M<-foreach(i=1:replication) %dopar%{
      optimization.GA(Al=Al,data=data,num=num,step=step,plot=FALSE)
    }
  }
  if(list==FALSE){
    para<-selection(M)
